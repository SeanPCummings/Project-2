...
function setUpClickEvents () {
  ...
  $('#saveBtn').on('click', function (event) {
    ...
    const name = $('#designName').val().trim();
    currentFace.name = name;
    currentFace.UserId = userid;

    if (name.length > 0) {
      savePumpkin();
      $('#pumpkin-saved-msg').html('<strong>Hey GOURD-geous!</strong> Your stencil was saved.');
      $('#designName').val('');
      $('#pumpkin-err-msg').empty();
    } else {
      $('#pumpkin-err-msg').text('Must provide a name');
      $('#pumpkin-saved-msg').empty();
    }
  });
